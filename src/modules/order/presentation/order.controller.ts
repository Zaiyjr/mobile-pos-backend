...
export class OrderController {
  ...
  checkout = asyncHandler(async (req: Request, res: Response) => {
    const employeeId = (req as Request & { user?: { userId: string } }).user?.userId;
  ...
  cancel = asyncHandler(async (req: Request, res: Response) => {
    res.status(200).json({ success: true, message: "ຍົກເລີກບິນຂາຍສຳເລັດ", data: await this.service.cancel(req.params.id as string) });
  });
}
